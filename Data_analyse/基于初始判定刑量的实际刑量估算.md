...
刑法对于犯罪入狱需要服刑的犯人的减刑策略作出了解释，但是认真遵守监视、接受教育改造等等都属于难以定量确定的因素，对于罪犯的真实刑期，即罪犯通过《刑法》第78条中规定的多种途径获得减刑后，真实服刑时长，法律是没有确切的说明的。因此，无法对影响因素进行量化分析，便通过对大量数据的统计分析来对实际刑量与判定刑量进行拟合。

### 研究方向

在不考虑罪犯的认错态度与是否初犯的情况下，进行实际刑量与初始判定量刑之间的定性分析与定量分析模拟，并进行结果的显著性检验。
